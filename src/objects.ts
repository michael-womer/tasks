...
export function makeBlankQuestion(
    ...
    return {id:id, name:name, type:type, expected:"", body:"", options:[], points:1, published:false};
}
...
export function isValid(question: Question, answer: string): boolean {
    return question.type === "short_answer_question" ? true : question.options.some((option:string):boolean => answer === option);
}
...
export function toMarkdown(question: Question): string {
    ...
    return "# " + question.name + "\n" + question.body + (question.type === "multiple_choice_question" ? question.options.map((option:string):string => 
        {return "\n- " + option}
).join("") : "");
}
...
export function publishQuestion(question: Question): Question {
    const oldPublished:boolean = question.published;
    return {... question, published:!oldPublished};
}
...
export function mergeQuestion(
    ...
    return {id:id, name:name, points:points, body:contentQuestion.body, type:contentQuestion.type, options:contentQuestion.options, expected:contentQuestion.expected, published:false};
}
